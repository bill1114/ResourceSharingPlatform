...
// it only stops casual/automated abuse. Real identity/permission checks are
// enforced server-side in the GAS backend via the signed login token.

const GAS_URL = 'https://script.google.com/macros/s/AKfycby_2FxJXc3f3xgdvCrSLluHFBgCiXVQGUMN_QudQo5eDdYomEPOdzdUMAoYIj8QK9AeVQ/exec';
const GAS_SECRET = 'ff8e9a96e27444709f4db29f919691db';
...
const Categories = ['食品', '醫療', '防護', '生活', '其他'];
...
function itemStatus(item) {
  ...
  if (Number(item.Quantity) <= Number(item.SafetyStock)) return { text: '低庫存', cls: 'bg-danger' };
...
function locationBadgeStyle(locationId) {
  const idx = ((Number(locationId) - 1) % LOCATION_PALETTE.length + LOCATION_PALETTE.length) % LOCATION_PALETTE.length;
...
async function callApi(action, payload) {
  ...
  if (json.success && json.data && json.data.token && action !== 'login') {
    // no-op placeholder (tokens are only ever issued by login)
  }
  return json;
}
...
async function guardedCall(action, payload) {
  ...
  if (!res.success && /登入已過期|未登入/.test(res.message || '')) {
    logout();
